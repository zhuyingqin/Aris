...
const EN_COPY = {
  ...
  rootDocumentsOnly: "Top-level folders holding .tex files",
  scanning: "Scanning local workspace…",
  documentCount: (count: number) => `${count} document${count === 1 ? "" : "s"}`,
  projectSummary: (projects: number, documents: number) => `${projects} project${projects === 1 ? "" : "s"} · ${documents} root document${documents === 1 ? "" : "s"}`,
  documentsInProject: (count: number) => `${count} root document${count === 1 ? "" : "s"}`,
  texFilesInProject: (count: number) => `${count} .tex file${count === 1 ? "" : "s"}`,
  refresh: "Refresh",
  ...
  openProject: (name: string) => `Open ${name} project`,
  toggleProject: (name: string, expanded: boolean) => `${expanded ? "Collapse" : "Expand"} ${name} project`,
...
const CN_COPY: typeof EN_COPY = {
  ...
  rootDocumentsOnly: "含 .tex 文件的一级文件夹",
  scanning: "正在扫描本地工作区…",
  documentCount: (count: number) => `${count} 个文档`,
  projectSummary: (projects: number, documents: number) => `${projects} 个项目 · ${documents} 个根文档`,
  documentsInProject: (count: number) => `${count} 个根文档`,
  texFilesInProject: (count: number) => `${count} 个 .tex 文件`,
  refresh: "刷新",
  ...
  openProject: (name: string) => `打开 ${name} 项目`,
  toggleProject: (name: string, expanded: boolean) => `${expanded ? "收起" : "展开"} ${name} 项目`,
  reveal: "在文件夹中显示",
