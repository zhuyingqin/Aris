...
export default function ComputePanel({
  ...
  return (
    <>
      <section className="lab-panel lab-runtime-panel lab-compute-node">
        <div className="lab-panel-head">
          <h3>{cn ? "计算目标" : "Compute target"}</h3>
          <button className="lab-btn ghost" type="button" onClick={() => void refresh()}>
            {cn ? "刷新" : "Refresh"}
          </button>
        </div>
        <select
          className="lab-select lab-compute-target-select"
          value={targetNodeId}
        ...
        <div className="lab-compute-target-meta">
          <span className={`lab-compute-online-dot${targetAvailable ? "" : " offline"}`} />
          <span>
            {targetNodeId === "local"
        ...
        <label className="lab-compute-field">
      ...
      <section className="lab-panel lab-runtime-panel">
        <div className="lab-panel-head">
          ...
          <span className="lab-count-badge">{jobs.length}</span>
        </div>
        {jobs.length === 0 ? (
          <div className="lab-muted">{cn ? "还没有计算任务。" : "No compute jobs yet."}</div>
        ) : (
          <div className="lab-compute-job-list">
            {jobs.slice(0, 12).map((job) => (
              <button
                type="button"
                key={job.request.jobId}
                className={`lab-compute-job${job.request.jobId === selectedJobId ? " active" : ""}`}
                onClick={() => setSelectedJobId(job.request.jobId)}
              >
                <span className={`lab-compute-status ${job.status}`}>{statusLabel(job.status, language)}</span>
        ...
        {selectedJob && (
          <div className="lab-compute-detail">
            <div className="lab-compute-detail-head">
              <code>{selectedJob.request.jobId.slice(0, 8)}</code>
              {!TERMINAL.has(selectedJob.status) && (
                <button className="lab-btn warn" type="button" disabled={busy} onClick={() => void cancelSelected()}>
                  {cn ? "取消任务" : "Cancel job"}
                </button>
              )}
            </div>
            {(stdout || stderr) ? (
              <pre className="lab-compute-log">
                {stdout}
                {stderr && <span className="lab-compute-stderr">{stderr}</span>}
              </pre>
            ) : (
              <div className="lab-muted">{cn ? "等待日志…" : "Waiting for logs…"}</div>
            )}
            {selectedJob.result?.error && <div className="lab-inline-error">{selectedJob.result.error}</div>}
            {selectedJob.result?.artifacts && selectedJob.result.artifacts.length > 0 && (
              <div className="lab-compute-artifacts">
