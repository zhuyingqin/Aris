...
function appendInlineMarkdown(target: HTMLElement, text: string): void {
  ...
  for (const match of text.matchAll(token)) {
    ...
    if (match[1] !== undefined) {
      ...
      const rawUrl = (match[3] ?? match[4] ?? "").trim();
      const src = safeRemoteImageUrl(rawUrl);
      if (src) {
        ...
        image.className = "remote-markdown-image";
        target.append(image);
      } else {
      ...
      const rawUrl = (match[10] ?? match[11] ?? "").trim();
      const href = safeRemoteUrl(rawUrl) ?? safeRemoteImageUrl(rawUrl);
      if (href) {
        const link = document.createElement("a");
        link.href = href;
        link.target = "_blank";
        link.rel = "noopener noreferrer";
        link.textContent = match[9];
        target.append(link);
      } else {
        appendPlainText(target, match[0]);
      }
      ...
      strong.textContent = match[12];
      target.append(strong);
    } else if (match[13] !== undefined) {
      const deleted = document.createElement("del");
      deleted.textContent = match[13];
      target.append(deleted);
    } else if (match[14] !== undefined || match[15] !== undefined) {
      const value = match[14] ?? match[15] ?? "";
...
export function safeRemoteImageUrl(value: string): string | null {
  ...
  let trimmed = value.trim();
  if (trimmed.startsWith("<") && trimmed.endsWith(">") && !trimmed.startsWith("<svg")) {
    trimmed = trimmed.slice(1, -1).trim();
  }
  // Allow base64 data URIs for images (png, jpeg, jpg, gif, webp, svg+xml, bmp, avif)
  if (/^data:image\/(?:png|jpe?g|gif|webp|svg\+xml|bmp|avif);base64,[A-Za-z0-9+/=\s]+$/i.test(trimmed)) {
    return trimmed.replace(/\s+/g, "");
  }
  // Allow SVG UTF-8 or URL-encoded data URIs
  if (/^data:image\/svg\+xml(?:;[a-z0-9=_-]+)?,/i.test(trimmed)) {
    return trimmed;
  }
  // Allow blob: URLs
