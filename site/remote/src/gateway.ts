...
import { accountHeadersForGateway, type AccountSession } from "./account";
import type {
    ...
    const claim = await this.request<ClaimPairingResponse>(
      ...
      `/v1/pairings/${encodeURIComponent(invitation.pairing_id)}/claims`,
      {
        ...
        headers: {
          ...
          ...(account ? accountHeadersForGateway(invitation.gateway_url, account) : {}),
        },
...
function validateClaimResponse(
  ...
  if (
    ...
    Math.abs(claim.expires_at_unix_ms - invitation.expires_at_unix_ms) > 120_000 ||
    claim.completion_expires_at_unix_ms <= nowUnixMs - 60_000
  ) {
...
export async function safeGatewayError(response: Response): Promise<string> {
  try {
