...
import { mobileBasePathUrl, resolveMobileBasePath } from "./basePath";
import {
  accountLoginUrl,
...
import { AccountGatewayApi, type AccountDeviceSummary } from "./accountGateway";
import {
  GatewayApi,
  GatewayApiError,
  type ClaimedPairing,
...
const MOBILE_BASE_PATH = resolveMobileBasePath(import.meta.env.BASE_URL, window.location.pathname);
const initialRemoteLocation = window.location.href;
const urlParams = new URLSearchParams(window.location.search);
const accountTokens = new AccountTokenManager({
  refreshUrl: accountRefreshUrl(siteRootUrl()),
});
/** Whether this browser carries a website sign-in at all. */
const hasAccountSession = accountTokens.peek() !== null;

/**
 * The account credential expected to be accepted *now*.
 *
 * Reading it once at boot was the reason a PWA left open (or resumed from the
 * background) kept presenting a token new-api had already expired: the value
 * is short-lived and is renewed in place, so every account call has to ask
 * for it again.
 */
async function currentAccountSession(): Promise<AccountSession> {
  return accountTokens.currentSession();
}

/**
 * Tells an embedding dashboard that a pairing finished.
 *
 * Without it the website could only infer success by polling the account
 * device list and watching for a name it had not seen before -- which cost a
 * request every two seconds and could not tell "paired" from "someone else
 * turned that computer on".
 */
function announcePairingComplete(session: PairedMobileSession): void {
  if (window.parent === window) return;
  window.parent.postMessage(
    {
      type: "somniq_pairing_complete",
      deviceId: session.invitation.desktop.device_id,
      deviceName: displayLabelForDesktop(session.invitation.desktop),
    },
...
/**
 * Renews on the way back from the background, so the first tap after a resume
 * does not pay for a round trip — and so a phone that sat idle for hours is
 * signed in again before the user asks for anything.
 */
function renewAccountSessionInBackground(): void {
  if (!hasAccountSession) return;
  void currentAccountSession().catch(() => undefined);
}
const requestedDesktopId = urlParams.get("desktop")?.trim() || null;
const isDesktopMode = urlParams.get("embed") === "1"
  || urlParams.get("mode") === "desktop"
  || (!/Mobile|Android|iPhone|iPad/i.test(navigator.userAgent) && window.innerWidth >= 768);
...
const themeParam = urlParams.get("theme") || localStorage.getItem("somniq_theme") || localStorage.getItem("somniq-site-theme");
applyTheme(themeParam);

window.addEventListener("message", (event) => {
  // Only the page that embedded this app may drive it, and it is same-origin
  // by construction: the dashboard serves both.
  if (event.origin !== window.location.origin) return;
  if (event.data && typeof event.data === "object" && event.data.type === "somniq_theme_change") {
...
const localChatDesignPreview =
  import.meta.env.DEV &&
  (window.location.hostname === "127.0.0.1" || window.location.hostname === "localhost") &&
  urlParams.get("preview") === "chat";
  ...
  <main class="remote-app" aria-labelledby="title">
    <header class="brand-header">
      ...
      <div>
        ...
        <h1 id="title">SomniQ Remote · 远程连接</h1>
      </div>
    </header>
    <p class="lede">安全连接同账号下的 SomniQ 电脑客户端，实时同步研究项目、历史对话与模型工作流。</p>

    <ol class="flow-steps" aria-label="远程连接步骤">
      <li data-flow-step="scan">1 选择设备</li>
      <li data-flow-step="approval">2 客户端确认</li>
      <li data-flow-step="connect">3 开始对话</li>
    </ol>

    <section id="scan-panel" class="card mobile-connect-card" hidden aria-labelledby="scan-title">
      <div class="card-heading mobile-connect-intro">
        <div>
          <p class="mobile-overline">同账号客户端</p>
          <h2 id="scan-title">选择要连接的电脑</h2>
          <p>已绑定的在线电脑会出现在这里。点击即可发起安全连接并在电脑端确认。</p>
        </div>
      </div>

      <section class="mobile-account-connect" aria-labelledby="account-devices-title">
        <div class="mobile-account-heading">
          <h3 id="account-devices-title">可用电脑设备</h3>
          <button id="refresh-account-devices" class="mobile-refresh-button" type="button">刷新</button>
        </div>
        <div id="account-device-list" class="mobile-account-device-list"></div>
        <p id="account-device-status" class="mobile-account-status" aria-live="polite"></p>
      </section>

      <div class="mobile-connect-divider"><span>首次绑定新电脑</span></div>

      <details id="first-pairing-details" class="mobile-first-pairing" ${window.innerWidth >= 600 ? "open" : ""}>
        <summary>绑定新电脑 (输入连接码 / 扫码) <span>展开</span></summary>
        <div class="mobile-first-pairing-body">
          <p>在电脑端 SomniQ「设置 → 远程访问」中点击「连接手机」，复制连接码直接粘贴在下方输入框，或使用手机扫码。</p>
          ...
          <p class="scan-or">或</p>
          <div class="pairing-code-entry">
            <textarea
              id="pairing-code"
              rows="2"
              placeholder="粘贴电脑上的连接码"
              autocomplete="off"
              spellcheck="false"
              aria-label="一次性连接码"
            ></textarea>
            <button id="use-pairing-code" class="secondary-button" type="button" disabled>使用连接码</button>
          </div>
          <p class="hint">二维码和连接码是同一份一次性凭证，只做端到端安全绑定，密钥仅保存在本机。</p>
        </div>
      </details>
    ...
    <section id="connected-panel" class="conversation-page" hidden aria-labelledby="connected-title">
      ...
      <aside id="workspace-drawer" class="workspace-drawer" aria-label="项目与对话列表" aria-hidden="true">
        ...
        <div id="workspace-drawer-content" class="workspace-drawer-content">
          <div class="workspace-sidebar-top-actions">
            <button id="create-chat-session" class="drawer-main-new-button" type="button" aria-label="新建对话" title="新建对话">
              <i data-lucide="plus" aria-hidden="true"></i><span>新建对话</span>
            </button>
            <button id="refresh-workspace" class="drawer-refresh-button" type="button" aria-label="刷新工作区与对话" title="刷新工作区与对话">
              <i data-lucide="refresh-cw" aria-hidden="true"></i>
            </button>
          </div>
          <div class="chat-sidebar-label chat-projects-label">远程项目</div>
...
// Account-plane calls renew a rejected credential and replay themselves.
const accountApi = new AccountGatewayApi(accountTokens);
...
const expandedProjectSessions = new Set<string>();
const COLLAPSED_SESSION_COUNT = 6;
...
const accountDeviceList = byId<HTMLElement>("account-device-list");
const accountDeviceStatus = byId<HTMLElement>("account-device-status");
const refreshAccountDevicesButton = byId<HTMLButtonElement>("refresh-account-devices");
const firstPairingDetails = byId<HTMLDetailsElement>("first-pairing-details");
...
const pairingCodeInput = byId<HTMLTextAreaElement>("pairing-code");
const usePairingCodeButton = byId<HTMLButtonElement>("use-pairing-code");
...
const refreshChatSessionsButton = document.getElementById("refresh-chat-sessions") as HTMLButtonElement | null;
const refreshWorkspaceButton = byId<HTMLButtonElement>("refresh-workspace");
const workspaceProjectsDetails = document.getElementById("drawer-projects-section") as HTMLDetailsElement | null;
const workspaceProjectCurrent = document.getElementById("workspace-project-current");
...
const workspaceSessionCount = document.getElementById("workspace-session-count");
...
refreshAccountDevicesButton.addEventListener("click", () => void loadAccountDevicesForLanding());
...
usePairingCodeButton.addEventListener("click", () => void usePairingCode());
pairingCodeInput.addEventListener("input", () => {
  usePairingCodeButton.disabled = pairingCodeInput.value.trim().length === 0;
});
pairingCodeInput.addEventListener("keydown", (event) => {
  if (event.key === "Enter" && !event.shiftKey && pairingCodeInput.value.trim()) {
...
chatForm.addEventListener("submit", (event) => {
  event.preventDefault();
  if (localChatDesignPreview) {
    sendLocalPreviewChat();
    return;
  }
...
document.addEventListener("visibilitychange", () => {
  ...
  renewAccountSessionInBackground();
});
...
if ("serviceWorker" in navigator && window.isSecureContext) {
  if (urlParams.get("embed") === "1" || isDesktopMode) {
    navigator.serviceWorker.getRegistrations().then((registrations) => {
      registrations.forEach((reg) => {
        if (reg.scope.includes("/remote/")) void reg.unregister();
      });
    }).catch(() => {});
  } else {
...
async function initialize(): Promise<void> {
  setPhase("loading");
  if (localChatDesignPreview) {
    showLocalChatDesignPreview();
    return;
  }
  if (!hasAccountSession) {
    rememberAuthReturnTo(initialRemoteLocation);
  ...
  if (restoration.restored && pairedSession) {
    ...
    if (requestedDesktopId) {
      const existing = pairedSessions.find(
        (session) => session.invitation.desktop.device_id === requestedDesktopId,
      );
      if (!existing) {
        addingDevice = true;
        await prepareAccountConnect(requestedDesktopId);
        return;
      }
      pairedSession = existing;
      updateDesktopLabels(existing);
    }
  ...
  if (scannedPairingPayload) {
    ...
    setStatus(
      ...
      "请选择一台同账号在线电脑；首次使用此手机时再展开扫码绑定。",
    );
    await loadAccountDevicesForLanding();
  }
}

async function prepareAccountConnect(desktopDeviceId: string): Promise<void> {
  if (!hasAccountSession) return;
  setPhase("loading");
  setStatus(isDesktopMode ? "正在向电脑端 SomniQ Studio 请求安全连接，请在电脑端弹窗中点击【允许】…" : "正在请求同账号客户端建立一次性安全连接…");
  const gatewayUrl = window.location.origin;
  try {
    let response = await accountApi.createConnectRequest(
      gatewayUrl,
      desktopDeviceId,
      directConnectClientLabel(),
    );
    while (response.status === "pending_desktop") {
      if (Date.now() >= response.expires_at_unix_ms) {
        throw new Error("客户端响应超时，请确认桌面端在线后重试。");
      }
      await new Promise((resolve) => window.setTimeout(resolve, 900));
      response = await accountApi.connectRequest(gatewayUrl, response.request_id);
    }
    if (!response.invitation) throw new Error("客户端未返回有效的一次性邀请。");
    scannedPairingPayload = JSON.stringify(response.invitation);
    clearDesktopConnectQuery();
    await claimPairing();
  } catch (error) {
    clearDesktopConnectQuery();
    setPhase("scan");
...
function directConnectClientLabel(): string {
  if (pairingContext) return `SomniQ Web · ${pairingBrowserContextLabel(pairingContext)}`;
  if (/Mobile|Android|iPhone|iPad/i.test(navigator.userAgent)) return "SomniQ Web · 手机浏览器";
  return "SomniQ Web · 桌面浏览器";
}

function clearDesktopConnectQuery(): void {
  const url = new URL(window.location.href);
  url.searchParams.delete("desktop");
  window.history.replaceState(null, "", `${url.pathname}${url.search}${url.hash}`);
}

async function loadAccountDevicesForLanding(): Promise<void> {
  if (!hasAccountSession) return;

  refreshAccountDevicesButton.disabled = true;
  accountDeviceStatus.textContent = "正在查找同账号电脑…";
  accountDeviceList.replaceChildren();

  try {
    const devices = await accountApi.devices(window.location.origin);
    devices.sort((left, right) => Number(right.online) - Number(left.online));

    if (devices.length === 0) {
      const empty = document.createElement("div");
      empty.className = "mobile-account-empty";
      empty.textContent = "还没有绑定过电脑，请使用下方二维码完成首次连接。";
      accountDeviceList.append(empty);
      firstPairingDetails.open = true;
      accountDeviceStatus.textContent = "";
      return;
    }

    for (const device of devices) {
      accountDeviceList.append(createAccountDeviceButton(device));
    }

    const onlineCount = devices.filter((device) => device.online).length;
    accountDeviceStatus.textContent = onlineCount > 0
      ? "点击在线设备后，请在电脑端核对并允许本次连接。"
      : "当前没有在线电脑。打开 SomniQ 客户端后点刷新。";
    firstPairingDetails.open = false;
  } catch (error) {
    firstPairingDetails.open = true;
    const credentialRejected =
      (error instanceof GatewayApiError && error.status === 401) ||
      (error instanceof AccountSessionError && error.reason !== "unavailable");
    if (credentialRejected) {
      // A rejected credential is not a transient failure: renewal has already
      // been tried and refused, so "刷新重试" would send the reader in a
      // circle. Redirecting on its own would loop whenever login returns here
      // without repairing the credential, so offer the way back explicitly
      // and let the reader decide.
      rememberAuthReturnTo(initialRemoteLocation);
      accountDeviceStatus.replaceChildren("登录状态已失效，读不到同账号电脑。");
      const relogin = document.createElement("a");
      relogin.className = "mobile-account-relogin";
      relogin.href = accountLoginUrl();
      relogin.textContent = "重新登录";
      accountDeviceStatus.append(" ", relogin);
      return;
    }
    accountDeviceStatus.textContent = "暂时无法读取设备列表，可刷新重试或使用扫码绑定。";
  } finally {
    refreshAccountDevicesButton.disabled = false;
  }
}

function createAccountDeviceButton(device: AccountDeviceSummary): HTMLButtonElement {
  const button = document.createElement("button");
  button.type = "button";
  button.className = "mobile-account-device";
  button.disabled = !device.online;
  button.setAttribute("aria-label", device.online ? "连接到 " + device.name : device.name + " 当前离线");

  const icon = document.createElement("span");
  icon.className = "mobile-account-device-icon";
  icon.textContent = "⌘";

  const copy = document.createElement("span");
  copy.className = "mobile-account-device-copy";
  const name = document.createElement("strong");
  name.textContent = device.name;
  const state = document.createElement("small");
  state.textContent = device.online ? "在线 · 等待连接" : "离线";
  copy.append(name, state);

  const action = document.createElement("span");
  action.className = "mobile-account-device-action";
  action.textContent = device.online ? "连接 →" : "—";

  button.append(icon, copy, action);
  if (device.online) {
    button.addEventListener("click", () => void prepareAccountConnect(device.id));
  }
  return button;
}
function showLocalChatDesignPreview(): void {
  setPhase("connected");
  workspaceDesktopName.textContent = "我的研究工作站";
  connectionDetail.textContent = "安全连接 · 24 ms";
  if (workspaceProjectCurrent) workspaceProjectCurrent.textContent = "AI Agent Research";
  currentProjectLabel.textContent = "AI Agent Research";
  currentSessionLabel.textContent = "多智能体评审框架";
  currentModelLabel.textContent = "GPT-5.6";
  connectedTitle.textContent = "多智能体评审框架";
  chatSessionStatus.textContent = "";
  if (workspaceSessionCount) workspaceSessionCount.textContent = "3";
  setStatus("本地设计预览 · 客户端已批准");

  activeProjectId = "proj-preview-1";
  workspaceProjects = [
    {
      projectId: "proj-preview-1",
    ...
    {
      projectId: "proj-preview-2",
      title: "Wind-CESN",
      phase: "实验已完成",
      updatedAtUnixMs: Date.now() - 86400000,
      activeRunId: null,
      isActive: false,
    },
  ];
  selectedChatSessionId = "sess-preview-1";
  chatSessions = [
    {
    ...
    },
    {
      ...
      title: "相关工作检索策略",
      updatedAtUnixMs: Date.now() - 3600000 * 2,
      model: "GPT-5.6",
    },
    {
      sessionId: "sess-preview-3",
      title: "实验结果与主张核对",
      updatedAtUnixMs: Date.now() - 86400000,
      model: "GPT-5.6",
    },
  ...
  appendChatMessage(
    "assistant",
    "桌面端的研究工作区已经连接。当前项目是 **AI Agent Research**，我已恢复「多智能体评审框架」这段对话。\n\n你可以继续发送指令，项目文件、会话历史和模型设置仍保留在电脑本地。",
    false,
    { force: true },
  );
  appendChatMessage(
    "user",
    "继续整理刚才的评审意见，并把需要补充的实验列出来。",
    false,
    { force: true },
  );
  appendChatMessage(
    "assistant",
    "已继续当前桌面会话。建议优先补充三项：独立 Reviewer 消融、长任务中断恢复，以及证据引用完整性检查。",
    false,
    { force: true },
  );

  updateLocalPreviewComposer();
  renderRemoteIcons();
  resizeChatComposer();
}

function sendLocalPreviewChat(): void {
  ...
  appendChatMessage(
    "assistant",
    "这是本地交互预览。正式连接后，这条消息会通过加密通道交给你的桌面 SomniQ，并在这里持续显示真实回复。",
    false,
    { force: true },
  );
  resizeChatComposer();
...
/**
 * Pairing path for a computer with no camera to point at the desktop QR.
 *
 * The desktop's QR and its copyable connection code carry the same one-time
 * invitation, so this reuses `pairingPayloadFromQrContent` and lands on the
 * same confirmation panel. Nothing about the desktop-side approval changes:
 * possession of the code still only buys the right to *ask*.
 */
function usePairingCode(): void {
  if (blockPairingInEphemeralContext()) {
    return;
  }
  const value = normalizePastedPairingCode(pairingCodeInput.value);
  if (!value) {
    return;
  }
  stopCameraScan();
  try {
    scannedPairingPayload = pairingPayloadFromQrContent(value);
  } catch (error) {
    setStatus(errorMessage(error));
    return;
  }
  // The code carries a single-use secret; do not leave it in the field.
  pairingCodeInput.value = "";
  usePairingCodeButton.disabled = true;
  showPairingConfirmation();
}
...
function showPairingConfirmation(): void {
  ...
  try {
    ...
    // Path-neutral: the same invitation may have arrived by camera or clipboard.
    pairingExpiry.textContent = `此次配对将在 ${formatExpiry(invitation.expires_at_unix_ms)} 前失效。`;
    setPhase("confirm");
...
async function claimPairing(): Promise<void> {
  ...
  try {
    ...
    const mobileIdentity = await WebCryptoMobileIdentity.loadOrCreate(
      ...
      await defaultPhoneName(),
    );
    const pending = await api.claimInvitation(
      scannedPairingPayload,
      mobileIdentity,
      MOBILE_P1_REQUESTABLE_SCOPES,
      // A QR ceremony is authorized by the signed invitation itself; the
      // account credential only enriches it, so an expired one must not
      // block a first pairing.
      await currentAccountSession().catch(() => null),
    );
...
async function completePairingWhenApproved(): Promise<void> {
  ...
  try {
    ...
    announcePairingComplete(session);
    await connect();
  } catch (error) {
  ...
  try {
    ...
    if (!overview || overview.projects.length === 0) {
      if (isUnsupportedRemoteCommand(response, "set_active_project")) {
        ...
        throw new Error("这台电脑尚不支持切换项目。请更新桌面端后重试。");
      }
    ...
    activeProjectId = project.projectId;
    resetRemoteChatState();
    if (workspaceProjectsDetails) workspaceProjectsDetails.open = false;
    renderWorkspaceProjects();
    renderChatWorkspaceHeader();
    updateChatComposer();
...
async function createChatSession(targetProjectId?: string): Promise<void> {
  ...
  try {
    if (projectId !== activeProjectId && supportsRemoteCapability("set_active_project")) {
      const switchResp = await sendControlRequest(newSetActiveProjectRequest(projectId));
      const overview = workspaceOverviewFromResponse(switchResp);
      if (overview && overview.projects.length > 0) {
        workspaceProjects = overview.projects;
        setWorkspaceCapabilities(overview);
    ...
    if (!created) {
      if (isUnsupportedRemoteCommand(response, "create_chat_session")) {
        ...
        throw new Error("当前电脑端版本尚不支持新建对话，请更新并重启 SomniQ Studio。");
      }
...
function renderWorkspaceProjects(): void {
  ...
  const canSwitchProjects = available && supportsRemoteCapability("set_active_project");
  ...
  if (workspaceProjects.length === 0) {
    ...
    placeholder.className = "chat-session-empty";
    placeholder.textContent = "正在读取电脑上的项目…";
    workspaceProjectsElement.append(placeholder);
    workspaceProjectsElement.scrollTop = prevScrollTop;
    return;
  }
  ...
  for (const project of workspaceProjects) {
    ...
    group.className = `chat-session-group workspace-project-group${isActive ? " selected" : ""}`;
    group.dataset.projectId = project.projectId;

    const label = document.createElement("div");
    label.className = "chat-sidebar-label chat-project-label";

    const toggle = document.createElement("button");
    toggle.type = "button";
    toggle.className = "chat-project-toggle";
    toggle.setAttribute("aria-expanded", isActive ? "true" : "false");
    toggle.title = isActive ? "当前活跃项目" : `切换到「${project.title}」`;
    toggle.addEventListener("click", () => {
      if (!isActive) {
        void selectWorkspaceProject(project.projectId);
      }
    });

    const caret = document.createElement("span");
    caret.className = "chat-project-caret";
    caret.setAttribute("aria-hidden", "true");
    const folderIcon = document.createElement("i");
    folderIcon.setAttribute("data-lucide", isActive ? "folder-open" : "folder");
    folderIcon.setAttribute("aria-hidden", "true");
    caret.append(folderIcon);

    const titleSpan = document.createElement("span");
    titleSpan.className = "chat-project-label-text";
    titleSpan.textContent = project.title;

    toggle.append(caret, titleSpan);

    if (isActive) {
      const currentTag = document.createElement("span");
      currentTag.className = "chat-remote-project-current";
      currentTag.textContent = "当前";
      toggle.append(currentTag);
    }

    const addBtn = document.createElement("button");
    addBtn.type = "button";
    addBtn.className = "chat-project-add";
    addBtn.textContent = "+";
    addBtn.title = `在「${project.title}」中新建对话`;
    addBtn.setAttribute("aria-label", `在「${project.title}」中新建对话`);
    addBtn.disabled = !canCreate || chatSessionCreating;
    addBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      void createChatSession(project.projectId);
    });

    label.append(toggle, addBtn);
    group.append(label);

    if (isActive) {
      const sessionsWrapper = document.createElement("div");
      sessionsWrapper.className = "chat-remote-project-sessions";
      sessionsWrapper.id = "workspace-session-list";

      if (!available) {
        const empty = document.createElement("div");
        empty.className = "chat-session-empty";
        empty.textContent = "请先连接电脑";
        sessionsWrapper.append(empty);
      } else if (chatSessionsLoading && chatSessions.length === 0) {
        const loading = document.createElement("div");
        loading.className = "chat-session-empty chat-remote-loading";
        const spinner = document.createElement("i");
        spinner.setAttribute("data-lucide", "loader-2");
        spinner.className = "spin";
        loading.append(spinner, document.createTextNode(" 正在读取会话…"));
        sessionsWrapper.append(loading);
      } else if (chatSessions.length === 0) {
        const empty = document.createElement("div");
        empty.className = "chat-session-empty";
        empty.textContent = canCreate ? "这个项目还没有对话。点击上方＋新建对话" : "没有可继续的对话";
        sessionsWrapper.append(empty);
      } else {
        const isExpanded = expandedProjectSessions.has(project.projectId);
        let visibleSessions: RemoteChatSession[] = chatSessions;

        if (!isExpanded && chatSessions.length > COLLAPSED_SESSION_COUNT) {
          const selectedIndex = chatSessions.findIndex((s) => s.sessionId === selectedChatSessionId);
          if (selectedIndex >= COLLAPSED_SESSION_COUNT) {
            visibleSessions = [
              ...chatSessions.slice(0, COLLAPSED_SESSION_COUNT - 1),
              chatSessions[selectedIndex],
            ];
          } else {
            visibleSessions = chatSessions.slice(0, COLLAPSED_SESSION_COUNT);
          }
        }
        ...
        for (const session of visibleSessions) {
          ...
          row.type = "button";
          const isSessionActive = session.sessionId === selectedChatSessionId;
          row.className = `chat-session-item workspace-session${isSessionActive ? " active" : ""}`;
          row.disabled = chatTranscriptLoading;
          row.title = session.title.trim() || "未命名对话";
          row.setAttribute("aria-current", isSessionActive ? "page" : "false");
          ...
          content.className = "chat-session-content workspace-session-content";

          const title = document.createElement("span");
          title.className = "chat-session-title workspace-session-title";
          title.textContent = session.title.trim() || "未命名对话";

          const updated = document.createElement("time");
          updated.className = "chat-session-updated workspace-session-updated";
          ...
          sessionsWrapper.append(row);
        }

        if (chatSessions.length > COLLAPSED_SESSION_COUNT) {
          const toggleBtn = document.createElement("button");
          toggleBtn.type = "button";
          toggleBtn.className = "chat-sessions-expand-btn";
          const span = document.createElement("span");
          const icon = document.createElement("i");
          if (isExpanded) {
            span.textContent = "收起历史对话";
            icon.setAttribute("data-lucide", "chevron-up");
            toggleBtn.addEventListener("click", () => {
              expandedProjectSessions.delete(project.projectId);
              renderWorkspaceProjects();
            });
          } else {
            const remainingCount = chatSessions.length - COLLAPSED_SESSION_COUNT;
            span.textContent = `展开更多历史对话 (${remainingCount})`;
            icon.setAttribute("data-lucide", "chevron-down");
            toggleBtn.addEventListener("click", () => {
              expandedProjectSessions.add(project.projectId);
              renderWorkspaceProjects();
            });
          }
          icon.setAttribute("aria-hidden", "true");
          toggleBtn.append(span, icon);
          sessionsWrapper.append(toggleBtn);
        }
      }
      group.append(sessionsWrapper);
    }

    workspaceProjectsElement.append(group);
  }
...
/**
 * Composer state for the local design preview, which has no transport and no
 * paired session.
 *
 * This must stay separate from the live gates below: those evaluate to "not
 * connected" here, so letting them run would disable the textarea on the first
 * keystroke — mid-word, dropping focus, leaving the typed character stranded
 * because the caret could no longer back over it.
 */
function updateLocalPreviewComposer(): void {
  chatInput.disabled = false;
  chatInput.placeholder = "向你的 SomniQ 发送消息…";
  ...
  chatHint.textContent = "端到端加密 · 数据保留在电脑本地";
}

function updateChatComposer(): void {
  if (localChatDesignPreview) {
    // Returning here also protects the preview's workspace and model labels:
    // the renderers at the end of this function read live session state and
    // would degrade them to "正在读取项目" / "先选择对话".
    updateLocalPreviewComposer();
    return;
  }
...
function defaultPhoneName(): Promise<string> {
  // `userAgentData` is Chromium-only; Safari and Firefox fall back to parsing.
  const hints = (navigator as Navigator & { userAgentData?: HighEntropyUserAgent })
    .userAgentData;
  return resolveClientDeviceName(navigator.userAgent, hints);
}
...
function blockPairingInEphemeralContext(): boolean {
  if (!pairingContext || isDesktopMode) {
    return false;
  }
  scannedPairingPayload = null;
  setPhase("scan");
  setStatus(
    `当前环境为${pairingBrowserContextLabel(pairingContext)}。内置浏览器退出后会隔离存储，影响持续会话。建议使用右上角“在浏览器打开”（Safari 或 Chrome），体验更稳定并支持添加到主屏幕。`,
  );
  return true;
}
...
function isEmbeddedWindow(): boolean {
  try {
    ...
    if (params.get("embed") === "1" || params.get("mode") === "desktop") {
      return false;
    }
    return isEphemeralEmbedding(window.top?.location.origin ?? null, window.location.origin);
  } catch {
    const params = new URLSearchParams(window.location.search);
    if (params.get("embed") === "1" || params.get("mode") === "desktop") {
      return false;
