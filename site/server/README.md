...
Managed-site pairing also requires the browser or phone to prove its NewAPI
account through a fixed, deployment-configured `/v1/user/self` endpoint. The
gateway discards the access token after verification and persists only a one-way
account digest. The desktop never forwards its account token: its signed QR
invitation plus the visible local approval remain the authority that binds the
first browser to the account and authorizes every later same-account connection.
...
cargo run --manifest-path site/server/Cargo.toml
```
...
| `SOMNIQ_GATEWAY_ACCOUNT_SELF_URL` | unset | Fixed NewAPI `/v1/user/self` introspection URL. When set, browser claims and account discovery require a valid same-account proof. |
| `SOMNIQ_GATEWAY_ACCOUNT_CONNECT_TTL_SECS` | `120` | Lifetime of a transient web-to-desktop connection request, 30-300 seconds. |
...
```powershell
cargo test --manifest-path site/server/Cargo.toml
```
