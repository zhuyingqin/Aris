...
7. `SOMNIQ_ACCOUNT_UPSTREAM` must answer `POST /api/user/auth/refresh` and
   `POST /api/user/auth/logout` in addition to the `/v1/auth/*` and
   `/v1/user/*` surface. new-api issues the browser a short-lived access token
   plus an HttpOnly refresh cookie scoped to that path; without the route the
   dashboard and the PWA cannot renew and start demanding a fresh password
   within minutes of every sign-in. Verify after deploying:

   ```bash
   curl -s -o /dev/null -w "%{http_code}\n" -X POST https://<your-host>/api/user/auth/refresh
   ```

   `401` is the expected answer for a cookie-less probe and means the route is
   wired. `404`, `405`, or an HTML body means the request never reached the
   account backend. When another edge (an existing Nginx, for example) owns
   the hostname and maps the `/v1/...` account paths itself, the same two
   endpoints have to be forwarded there as well:
   ...
   Keep the location this narrow. The rest of `/api/user/*` is new-api's
   management surface and must not become reachable from the public site.
