...
import { independentAccountsEnabled, loadIndependentAccount, type IndependentAccount } from "./independentAccount";
import {
...
function PlanEditor({ plan, knownModels, busy, onSave, onDiscover }: {
  ...
  return <form className="admin-panel" onSubmit={event => { event.preventDefault(); void onSave({
    ...
    <div className="admin-save-row"><label className="admin-switch"><input data-testid="plan-enabled" type="checkbox" checked={enabled} disabled={busy} onChange={e => setEnabled(e.target.checked)} /><span>启用此版本的模型权限</span></label>
      <button className="identity-primary" type="submit" disabled={busy} data-testid="save-plan">{busy ? "正在保存…" : "保存并生效"}</button></div>
  </form>;
}
...
function MemberEditor({ user, plans, busy, onSave }: { user: MemberUser; plans: MembershipPlan[]; busy: boolean;
  ...
  return <form className="admin-panel" onSubmit={e => { e.preventDefault(); void onSave(plan || null, plan ? Math.floor(new Date(expires).getTime() / 1000) : null, reason); }}>
    ...
    <button type="submit" className="identity-primary" data-testid="save-membership" disabled={busy || !user.active}>{busy ? "正在保存…" : "保存会员"}</button>
  </form>;
}
...
export default function AdminApp() {
  ...
  useEffect(() => {
    document.documentElement.lang = "zh-CN"; document.title = "SomniQ 管理员";
    if (!independentAccountsEnabled) { setLoading(false); return; }
    let live = true;
    loadIndependentAccount().then(async value => {
      if (!live) return; setAccount(value);
      if (value?.is_admin) await reload();
    }).catch(e => { if (live) setError(membershipError(e)); }).finally(() => { if (live) setLoading(false); });
    return () => { live = false; };
  }, []);
  ...
  return <div className="identity-page admin-page">
    <header className="identity-nav admin-nav"><a href="./" className="identity-brand"><img src="./app-logo.png" alt="" width="32" height="32" />SomniQ <span className="admin-label">ADMIN</span></a><div><a href="./account.html">账户中心 ↗</a><a href="./">返回首页</a></div></header>
    <main className="admin-main">
      <div className="admin-title-row"><div><span className="admin-overline">SOMNIQ CONTROL CENTER</span><h1>会员与模型管理</h1><p>管理 Go、Plus、Pro 的模型权限与用户会员。</p></div>{account?.is_admin && <button disabled={busy} onClick={() => void run(reload)}>刷新数据</button>}</div>
      {error && <p role="alert" className="identity-error">{error}</p>}{notice && <p role="status" className="admin-notice">{notice}</p>}
      {loading ? <p role="status">正在验证管理员身份…</p> : !independentAccountsEnabled ? <section className="admin-panel"><h2>此部署尚未启用独立账号</h2><p>请通过已配置的 SomniQ 账号服务部署管理员入口。</p></section> : !account ? <section className="admin-panel"><h2>登录管理员账号</h2><p>使用已授权的 SomniQ 账号登录，再进入管理员后台。</p><a className="identity-primary" href="./account.html">前往登录 →</a></section> : !account.is_admin ? <section className="admin-panel"><h2>此账号没有管理员权限</h2><p>管理员由服务器授权。修改本地设置不会改变账号权限。</p><a href="./account.html">返回账户中心 →</a></section> : <>
        ...
        {tab === "users" && <div className="admin-users-layout"><section className="admin-panel"><form className="admin-search" onSubmit={e => { e.preventDefault(); void run(() => search(0)); }}><input aria-label="搜索用户" placeholder="邮箱、姓名或账户 ID" value={query} onChange={e => setQuery(e.target.value)} maxLength={200} /><button disabled={busy}>搜索</button></form>
        ...
        {tab === "audit" && <section className="admin-panel"><div className="admin-panel-heading"><h2>最近的操作记录</h2><span className="admin-revision">最近 100 条</span></div>{events.length === 0 ? <p className="admin-description">尚无变更记录。</p> : <div className="admin-audit-list">{events.map(event => <details key={event.id}><summary><strong>{event.action === "plan.updated" ? "更新套餐模型" : "变更用户会员"}</strong><span>{event.actor_email}</span><time>{new Date(event.created_at * 1000).toLocaleString()}</time></summary><p>对象：{event.target}</p>{event.reason && <p>原因：{event.reason}</p>}<div className="admin-form-grid"><div><h3>变更前</h3><pre>{JSON.stringify(JSON.parse(event.before), null, 2)}</pre></div><div><h3>变更后</h3><pre>{JSON.stringify(JSON.parse(event.after), null, 2)}</pre></div></div></details>)}</div>}</section>}
      </>}
