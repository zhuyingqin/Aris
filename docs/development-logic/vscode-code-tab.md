...
改成固定端口段（服务端取段内最低空闲端口），只有端口被占用时才会漂移并重置状态。

**端口段必须落在 Windows 动态端口范围（默认 49152-65535）之下。**
最初选的 `52411-52430` 在那个范围里，2026-09-11 整段被
Hyper-V/WinNAT 的一条 `52381-52480` 保留吃掉：这类保留是成百端口一块地划走的，
划走之后 Windows 连显式 bind 都拒，`netstat` 里却什么都没监听（没有进程占用它），
而且每次重启保留的偏移都会变——所以那种端口段是"能用到某一天为止"。
现为 `42411-42430`；`netsh interface ipv4 show excludedportrange protocol=tcp`
是查这类保留的入口。服务端在这种情况下只往 **stderr** 写
`Could not find free port in range`，`spawn_server` 因此保留最后一行 stderr 并带进错误里，
否则 UI 上只剩一个退出码。
