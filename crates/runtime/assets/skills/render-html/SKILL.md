...
allowed-tools: read_file, write_file, bash, LlmReview
---
...
- **Cross-model review at the artifact boundary** (ARIS invariant). Academic-template HTML — used for the artifacts humans actually read (IDEA_REPORT, AUTO_REVIEW, KILL_ARGUMENT, PAPER_PLAN) — is reviewed by a fresh cross-family reviewer call before being claimed as a finished view. Dashboard-template HTML (cockpit / debug views) skips review by default but accepts `--review` to force it. See § *HTML Review Gate* below.
...
RENDER_HTML=""
for candidate in \
  "$HOME/.config/SomniQ/skills/render-html/scripts/render_html.py" \
  "${ARIS_CACHE_DIR:-.}/skills/render-html/scripts/render_html.py" \
  "skills/render-html/scripts/render_html.py"; do
  [ -f "$candidate" ] && { RENDER_HTML="$candidate"; break; }
done
[ -z "$RENDER_HTML" ] && {
  echo "ERROR: render_html.py not resolved. Checked ~/.config/SomniQ/, \$ARIS_CACHE_DIR/, and ./skills/." >&2
  echo "       /render-html cannot produce HTML output. Fix: reinstall SomniQ so the bundled assets extract, or drop a copy at ~/.config/SomniQ/skills/render-html/scripts/." >&2
  exit 1
}
```
...
python3 "$RENDER_HTML" idea-stage/IDEA_REPORT.md
# … then the skill skips the LlmReview review step if --no-review
...
**Decide whether to run review.** Per ARIS invariant "executor must not judge its own output", the academic-template HTML is reviewed by a fresh cross-family reviewer call before being claimed as a delivered view. Resolution:
...
**If `should_review` is true**, fire a fresh `LlmReview` thread (NEVER a continued reviewer thread) with the prompt below. The reviewer reads the source MD + generated HTML directly; it does **not** see this skill's intermediate state.
...
**Reviewer prompt (mandatory shape).** Send this as a fresh thread (`LlmReview`, NOT a continued reviewer thread):
...
2. Save the raw reviewer trace to `.aris/traces/render-html/<YYYY-MM-DD>_run<NN>/review.{txt,json}` per `shared-references/review-tracing.md`.
...
**If `LlmReview` is not available** (e.g., user runs `/render-html` on a Reviewer-CLI-only setup where `LlmReview` isn't wired): emit `verdict: REVIEW_UNAVAILABLE` to the sidecar, do not fabricate `PASS`, and tell the user the HTML was generated but **not** independently reviewed. The user can manually invoke `/research-review` on the source MD or re-run with `LlmReview` available.
...
For deck / poster / Xiaohongshu card / tweet card / data report style outputs, point users to **[html-anything](https://github.com/nexu-io/html-anything)** (Apache-2.0, 3000⭐ at time of writing). It ships 75 SKILL.md templates across 9 surfaces and detects 8 coding-agent CLIs (including Claude Code, Reviewer, Copilot). ARIS does *not* depend on it — `/render-html` covers ARIS-native artifacts; html-anything is the recommended path for richer publishing surfaces.
