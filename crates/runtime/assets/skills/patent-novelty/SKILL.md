---
name: patent-novelty
description: "Search patent databases and literature for prior art, then assess novelty and non-obviousness against it. Use when user says \"现有技术检索\", \"prior art search\", \"专利检索\", \"专利查新\", \"patent novelty\", \"可专利性评估\", \"patentability check\", or wants to know whether an invention is patentable."
argument-hint: "[invention-description-or-brief-path] [— stage: search|assess|all]"
allowed-tools: read_file, write_file, edit_file, glob_search, grep_search, bash, WebSearch, WebFetch, LlmReview, Agent
---

# Patent Novelty: Prior Art Search + Patentability Assessment

Assess whether an invention is patentable: **$ARGUMENTS**

Two stages, run in order. Stage 2 consumes stage 1's output, so a standalone
`assess` run requires `patent/PRIOR_ART_REPORT.md` to already exist.

| Stage | Does | Writes |
|-------|------|--------|
| `search` | Searches patent databases + academic literature for prior art | `patent/PRIOR_ART_REPORT.md` |
| `assess` | Tests the invention against that prior art under patent law | `patent/NOVELTY_ASSESSMENT.md` |

**Default (`— stage: all`)**: run `search`, then `assess`.
Pass `— stage: search` or `— stage: assess` to run one only.
...
- `REVIEWER_MODEL = configured reviewer` — Used via `LlmReview` for cross-model examiner verification
- `NOVELTY_STANDARD = patent` — Always the legal patentability standard, never the research-contribution standard

## Inputs

1. Invention description from `$ARGUMENTS`
2. `patent/INVENTION_BRIEF.md` if it exists (or `INVENTION_BRIEF.md` at project root)
3. `patent/PRIOR_ART_REPORT.md` — required input for `assess`, produced by `search`
...
Load `../shared-references/patent-format-us.md` for the 102/103 analysis framework.

---

## Stage 1 — `search`: Prior Art Search

Adapted from `/research-lit` for patent-specific searching.

### Step 1.1: Extract Search Concepts
...
### Step 1.5: Freedom-to-Operate Assessment (Preliminary)
...
### Step 2.2: Anticipation Analysis (Novelty)
...
### Step 2.3: Obviousness Analysis (Inventive Step)

If the invention is novel (passes Step 2.2), test for obviousness:
...
### Step 2.4: Cross-Model Examiner Verification
...
### Step 2.5: Jurisdiction-Specific Assessment
...
- If `LlmReview` is not available, skip the cross-model examiner review and note it in the output.
