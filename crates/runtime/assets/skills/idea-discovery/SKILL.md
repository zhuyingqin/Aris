...
- **REVIEWER_MODEL = `configured reviewer`** — Model used via `LlmReview`. Set in SomniQ Settings; omit the `model` field when calling. Passed to sub-skills.
...
- Brainstorm 8-12 concrete ideas via the reviewer
...
- Cross-verify with the reviewer
...
**What this does:**
- the reviewer acts as a senior reviewer (NeurIPS/ICML level)
...
- Iteratively refine the method via reviewer review rounds (up to 5 rounds, until score ≥ 9)
