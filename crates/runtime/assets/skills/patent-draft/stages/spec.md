# Stage: `spec` — Specification Writing

Write the patent specification section by section. Adapted from `/paper-write`,
but the specification supports the claims — it is not a paper.

The detailed description (具体实施方式) is produced by the `embodiments` stage;
run that first, or run it as part of Step 7 below.

## Inputs

1. `patent/CLAIMS.md` — the drafted claims (primary source)
2. `patent/INVENTION_DISCLOSURE.md` — invention decomposition
3. `patent/PRIOR_ART_REPORT.md` — for the background section
4. `patent/figures/` — figure descriptions and numeral index
...
Load `../shared-references/patent-format-cn.md`, `patent-format-us.md`, or `patent-format-ep.md` based on jurisdiction.
...
Use the output of the `figures` stage (`patent/figures/figure_descriptions.md`).
If that stage has not run and the user has figures, run it now.

If there are no user figures, describe what figures should exist based on the claims.
...
Use the output of the `embodiments` stage. If that stage has not run, run it now.
