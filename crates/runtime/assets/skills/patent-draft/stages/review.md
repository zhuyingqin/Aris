# Stage: `review` — Examiner Review

Two rounds of examiner-style review of the drafted application. Adapted from
`/research-review`, but the reviewer persona is a **patent examiner**, not a
paper reviewer.

## Stage Constants

- `REVIEW_ROUNDS = 2` — Number of review rounds
- `EXAMINER_PERSONA = "patent-examiner"` — The reviewer persona for every call in this stage
...
- Address CRITICAL and MAJOR issues before proceeding to the `format` stage.
