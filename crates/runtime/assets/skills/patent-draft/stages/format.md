# Stage: `format` — Jurisdiction Filing Compilation

Compile the finished application into filing-ready format. Analogous to
`/paper-compile`, but for patent document formatting instead of LaTeX.

**This stage does NOT modify claim content — it reformats existing content only.**
...
From `$ARGUMENTS` or the `JURISDICTION` constant:
