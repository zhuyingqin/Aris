...
- REVIEWER_MODEL = `configured reviewer` — used via `LlmReview` for ambiguous cases only
...
Only escalate to Reviewer when the signal is ambiguous. For clearly good or clearly bad signals, act directly.
