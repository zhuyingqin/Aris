...
impl FocusSignals {
    ...
    pub fn nudge(&self) -> Option<String> {
        ...
        let demand = if self.has_repeated_failure() {
            "Do not retry the same failure. Change mechanism once, or deliver what was tried, \
             what remains blocked, and what is needed."
        } else {
            "Compare the work with this turn's completion checks. If they pass, deliver now. \
             Otherwise name the unmet check and take one necessary next step; if the approach is \
             not converging, switch it or report the blocker."
        };
        Some(format!(
