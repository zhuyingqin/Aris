...
/// Fields of a JSON tool payload that carry the actual failure text, most
/// specific first. Tool results are pretty-printed JSON, so the raw first line
/// is `{` for every failure of every tool; without this, four unrelated `bash`
/// failures share one signature and look like one unresolved loop.
const JSON_ERROR_FIELDS: [&str; 9] = [
    "stderr",
    "error",
    "evalue",
    "message",
...
/// Every Python traceback opens with this line, so it identifies nothing.
...
impl FocusSignals {
    ...
    /// Whether the same failure has come back often enough that retrying it is
    /// demonstrably not converging. Unlike the other thresholds this is not a
    /// statement about narrowness: the approach has already been tried and has
    /// already produced this result.
    #[must_use]
    pub fn has_repeated_failure(&self) -> bool {
        self.top_error
            .as_ref()
            .is_some_and(|(_, count)| *count >= RABBIT_HOLE_ERROR_REPEATS)
    }

    /// The in-band reminder text, or `None` when no threshold is crossed.
    ///
    /// For the narrowness thresholds the wording deliberately asks for a
    /// decision rather than ordering a stop: the counters prove narrowness, not
    /// wrongness, and a long stretch on one file is often exactly right. A
    /// repeated identical failure is different — restating that the work is on
    /// the main line changes nothing when the same attempt keeps producing the
    /// same error — so that case asks for a changed approach or a stop.
    #[must_use]
    pub fn nudge(&self) -> Option<String> {
        ...
        let demand = if self.has_repeated_failure() {
            "A failure that keeps coming back unchanged is evidence the current approach is not \
             converging, so repeating it is not progress. On the next step either change approach \
             materially — a different mechanism, not the same one with another parameter — or stop \
             and report what was tried, what failed each time, and what you need from the user. \
             Do not retry the same thing again."
        } else {
        ...
        Some(format!(
            "Main-line check (generated by Aris from the session, not by the user): {}. {demand}",
            reasons.join("; ")
        ))
    }
}

/// What a tool call targeted: the file, plus the region or cell when the tool
/// named one. Reads carry an `offset` and notebook tools carry a cell, so
/// paging through a long file or working down a notebook cell by cell produces
/// many distinct targets — ordinary work — while edits (which name neither) and
/// repeated visits to one region collapse onto a single target, which is the
/// narrow-focus signal.
...
impl FileTarget {
    /// Several path and cell spellings are accepted because MCP tools do not
    /// share the built-ins' `path` convention.
    fn from_tool_input(input: &str) -> Option<Self> {
        ...
        let path = ["path", "file_path", "filePath", "notebook_path"]
            ...
            .to_string();
        let cell = ["cell_index", "cellIndex", "cell_id", "cellId"]
            .iter()
...
fn error_signature(tool_name: &str, output: &str) -> Option<String> {
    ...
    let salient = detail
        .as_deref()
        .and_then(salient_error_line)
        .or_else(|| salient_error_line(output))?;
    let mut normalized = String::with_capacity(salient.len());
    let mut last_was_digit = false;
    for character in salient.chars() {
...
/// The failure text buried inside a JSON tool payload. Every built-in tool
/// serializes its result as a JSON object, so the failure never appears on the
/// blob's first line — reading the blob as plain text yields `{` for all of
/// them and collapses every distinct failure onto one signature.
fn json_error_detail(output: &str) -> Option<String> {
    let value = serde_json::from_str::<serde_json::Value>(output).ok()?;
    let object = value.as_object()?;
    // A notebook cell reports its failure as a structured output block, where
    // the exception name and value are the exact identity of the failure.
    if let Some(detail) = notebook_error_detail(object) {
        return Some(detail);
    }
    JSON_ERROR_FIELDS.iter().find_map(|key| {
        object
            .get(*key)
...
fn notebook_error_detail(object: &serde_json::Map<String, serde_json::Value>) -> Option<String> {
    object
        .get("outputs")?
        .as_array()?
        .iter()
        ...
        .find_map(|output| {
            let ename = output.get("ename").and_then(serde_json::Value::as_str)?;
            let evalue = output
                .get("evalue")
                .and_then(serde_json::Value::as_str)
                .unwrap_or_default();
            Some(format!("{ename}: {evalue}").trim().to_string())
        })
}

/// The line of a failure blob that identifies *which* failure it is. Structural
/// noise is skipped because it is identical across failures, and a Python
/// traceback is read from the end: every traceback opens with the same header
/// and names its exception on the last line.
fn salient_error_line(text: &str) -> Option<&str> {
    let lines = text
        .lines()
        .map(str::trim)
        .filter(|line| !line.is_empty())
        .collect::<Vec<_>>();
    if lines.iter().any(|line| line.starts_with(TRACEBACK_HEADER)) {
        if let Some(last) = lines
            .iter()
            .rev()
            .copied()
            .find(|line| !is_structural_noise(line))
        {
            return Some(last);
        }
    }
    lines.into_iter().find(|line| !is_structural_noise(line))
}

/// Whether a line carries no failure-identifying content: JSON punctuation,
/// separator rules, and the fixed traceback header are the same for every
/// failure of every tool.
fn is_structural_noise(line: &str) -> bool {
    !line.chars().any(char::is_alphanumeric) || line.starts_with(TRACEBACK_HEADER)
}
